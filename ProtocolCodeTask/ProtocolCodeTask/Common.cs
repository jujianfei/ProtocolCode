...
namespace ProtocolCodeTask
{
    ...
    public class Common
    {
        ...
        public static DateTime? ToDateTime(object dateNum, object timeNum)
        {
            ...
            {
                int intDateNum = Common.ToInt(dateNum);
                int intTimeNum = Common.ToInt(timeNum);
        ...
        /// <param name="start">包括此位</param>
        /// <param name="len">要计算和校验的位数长度</param>
        /// <returns>和校验数值</returns>
        public static byte CalcCSCommon(byte[] data, int start, int len)
        {
            byte cs = 0;
        ...
        #region << 获取枚举对象的指定特性(返回为null代表枚举对象无此特性) >>
        /// <summary>
        /// 获取枚举对象的指定特性(返回为null代表枚举对象无此特性)
        /// </summary>
        /// <typeparam name="T">特性</typeparam>
        /// <param name="e">枚举对象</param>
        /// <returns>枚举对象的指定特性(null代表枚举对象无此特性)</returns>
        public static T GetEnumAttribute<T>(Enum e) where T : Attribute
        {
            T Attr = null;
            object[] objAttrs = e.GetType().GetField(e.ToString()).GetCustomAttributes(typeof(T), false);
            if (objAttrs != null && objAttrs.Length > 0)
                Attr = objAttrs[0] as T;
            return Attr;
        }
        #endregion
    }
